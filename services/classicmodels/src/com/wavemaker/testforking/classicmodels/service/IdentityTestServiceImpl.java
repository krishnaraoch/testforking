...
@Validated
@EntityService(entityClass = IdentityTest.class, serviceId = "classicmodels")
public class IdentityTestServiceImpl implements IdentityTestService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public IdentityTest partialUpdate(String identitytestId, Map<String, Object>identityTestPatch) {
        LOGGER.debug("Partially Updating the IdentityTest with id: {}", identitytestId);

        IdentityTest identityTest = getById(identitytestId);

        try {
            ObjectReader identityTestReader = this.objectMapper.reader().forType(IdentityTest.class).withValueToUpdate(identityTest);
            identityTest = identityTestReader.readValue(this.objectMapper.writeValueAsString(identityTestPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", identityTestPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        identityTest = update(identityTest);
