...
@Validated
@EntityService(entityClass = Offices.class, serviceId = "classicmodels")
public class OfficesServiceImpl implements OfficesService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public Offices partialUpdate(String officesId, Map<String, Object>officesPatch) {
        LOGGER.debug("Partially Updating the Offices with id: {}", officesId);

        Offices offices = getById(officesId);

        try {
            ObjectReader officesReader = this.objectMapper.reader().forType(Offices.class).withValueToUpdate(offices);
            offices = officesReader.readValue(this.objectMapper.writeValueAsString(officesPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", officesPatch, ex);
