...
@Validated
@EntityService(entityClass = Mytable.class, serviceId = "classicmodels")
public class MytableServiceImpl implements MytableService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public Mytable partialUpdate(MytableId mytableId, Map<String, Object>mytablePatch) {
        LOGGER.debug("Partially Updating the Mytable with id: {}", mytableId);

        Mytable mytable = getById(mytableId);

        try {
            ObjectReader mytableReader = this.objectMapper.reader().forType(Mytable.class).withValueToUpdate(mytable);
            mytable = mytableReader.readValue(this.objectMapper.writeValueAsString(mytablePatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", mytablePatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        mytable = update(mytable);
