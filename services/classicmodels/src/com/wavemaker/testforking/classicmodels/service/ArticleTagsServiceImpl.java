...
import com.wavemaker.testforking.classicmodels.ArticleTags;
import com.wavemaker.testforking.classicmodels.ArticleTagsId;
...
@Validated
@EntityService(entityClass = ArticleTags.class, serviceId = "classicmodels")
public class ArticleTagsServiceImpl implements ArticleTagsService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public ArticleTags partialUpdate(ArticleTagsId articletagsId, Map<String, Object>articleTagsPatch) {
        LOGGER.debug("Partially Updating the ArticleTags with id: {}", articletagsId);

        ArticleTags articleTags = getById(articletagsId);

        try {
            ObjectReader articleTagsReader = this.objectMapper.reader().forType(ArticleTags.class).withValueToUpdate(articleTags);
            articleTags = articleTagsReader.readValue(this.objectMapper.writeValueAsString(articleTagsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", articleTagsPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        articleTags = update(articleTags);
