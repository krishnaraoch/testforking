...
@Validated
@EntityService(entityClass = Orderdetails.class, serviceId = "classicmodels")
public class OrderdetailsServiceImpl implements OrderdetailsService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public Orderdetails partialUpdate(OrderdetailsId orderdetailsId, Map<String, Object>orderdetailsPatch) {
        LOGGER.debug("Partially Updating the Orderdetails with id: {}", orderdetailsId);

        Orderdetails orderdetails = getById(orderdetailsId);

        try {
            ObjectReader orderdetailsReader = this.objectMapper.reader().forType(Orderdetails.class).withValueToUpdate(orderdetails);
            orderdetails = orderdetailsReader.readValue(this.objectMapper.writeValueAsString(orderdetailsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", orderdetailsPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        orderdetails = update(orderdetails);
