...
@Validated
@EntityService(entityClass = Orders.class, serviceId = "classicmodels")
public class OrdersServiceImpl implements OrdersService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public Orders partialUpdate(Integer ordersId, Map<String, Object>ordersPatch) {
        LOGGER.debug("Partially Updating the Orders with id: {}", ordersId);

        Orders orders = getById(ordersId);

        try {
            ObjectReader ordersReader = this.objectMapper.reader().forType(Orders.class).withValueToUpdate(orders);
            orders = ordersReader.readValue(this.objectMapper.writeValueAsString(ordersPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", ordersPatch, ex);
