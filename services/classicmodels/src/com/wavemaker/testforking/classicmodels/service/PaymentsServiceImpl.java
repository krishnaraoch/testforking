...
@Validated
@EntityService(entityClass = Payments.class, serviceId = "classicmodels")
public class PaymentsServiceImpl implements PaymentsService {
    ...
    @Transactional(value = "classicmodelsTransactionManager")
    @Override
    public Payments partialUpdate(PaymentsId paymentsId, Map<String, Object>paymentsPatch) {
        LOGGER.debug("Partially Updating the Payments with id: {}", paymentsId);

        Payments payments = getById(paymentsId);

        try {
            ObjectReader paymentsReader = this.objectMapper.reader().forType(Payments.class).withValueToUpdate(payments);
            payments = paymentsReader.readValue(this.objectMapper.writeValueAsString(paymentsPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", paymentsPatch, ex);
